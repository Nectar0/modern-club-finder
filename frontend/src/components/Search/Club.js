...
const Club = (props) => {
  ...
  const clubsShownArray = clubsShownValues
    .filter((activity) => {
      ...
      if (categories[0] !== "") {
        categoryMatch = activity.categories.some((category) =>
      ...
      if (availableDays[0] !== "") {
        dayMatch = activity.meetingDays.some((day) =>
  ...
  return (
    <div className=" div-margin ml-10 mr-10 grid grid-cols-1 phone: grids-cols-2 gap-2 ml-10 mr-10 tablet:grid-cols-2 gap-2 ml-10 mr-10 laptop:grid-cols-3 gap-2 ml-10 mr-10 desktop:grid-cols-3 gap-5 ml-10 mr-10">
      {reusableClubArray}
      {showModal && (
        <Modal
